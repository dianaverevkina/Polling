...
function concatMap(project, resultSelector) {
    return isFunction_isFunction(resultSelector) ? mergeMap(project, resultSelector, 1) : mergeMap(project, 1);
}
...
;// CONCATENATED MODULE: ./src/js/Polling.js
...
class Polling {
  ...
  subscribeOnNewMessages() {
    ...
    this.subscription = interval$.pipe(concatMap(() => ajax.getJSON('http://localhost:3000/messages/unread')), catchError(() => of({
      messages: []
    }))).subscribe(response => {
